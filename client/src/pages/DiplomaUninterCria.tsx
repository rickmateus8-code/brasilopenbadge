  ...
  return (
    <DashboardLayout>
      ...
      <div className="uninter-form">
        ...
        {/* SEÇÃO 3: REGISTRO E EMISSÃO */}
        <div className="uninter-divider">📅 3. REGISTRO E EMISSÃO</div>
        <div className="uninter-grid">
        ...
        {/* SEÇÃO 4: ASSINATURAS */}
        <div className="uninter-divider">🖋️ 4. ASSINATURAS</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 20 }}>
          {/* Assinatura Secretária */}
          <div style={{ padding: 15, border: "1px solid #e2e8f0", borderRadius: 12 }}>
            <label style={{ fontSize: 12, fontWeight: 700, display: "block", marginBottom: 10 }}>Secretário(a) Geral</label>
            <input type="text" value={data.secretaria_nome} onChange={update("secretaria_nome")} style={{ width: "100%", marginBottom: 10, padding: 8, borderRadius: 6, border: "1px solid #ddd" }} />
            <div style={{ display: "flex", gap: 5, marginBottom: 10 }}>
              <input type="text" placeholder="Digitar nome para ass..." value={assTexto} onChange={(e) => setAssTexto(e.target.value)} style={{ flex: 1, padding: 6, fontSize: 12 }} />
              <button className="uninter-btn-action" style={{ background: "#6366f1" }} onClick={() => gerarAssinatura("assSecretariaImg")}>Assinar</button>
            </div>
            <label className="uninter-btn-action" style={{ background: "#94a3b8" }}>
              <Upload size={12} /> Upload Imagem
              <input type="file" accept="image/*" style={{ display: "none" }} onChange={handleUploadImg("assSecretariaImg")} />
            </label>
            {data.assSecretariaImg && <img src={data.assSecretariaImg} style={{ height: 40, marginTop: 10, border: "1px solid #eee" }} />}
          </div>

          {/* Assinatura Reitor */}
          <div style={{ padding: 15, border: "1px solid #e2e8f0", borderRadius: 12 }}>
            <label style={{ fontSize: 12, fontWeight: 700, display: "block", marginBottom: 10 }}>Reitor</label>
            <input type="text" value={data.reitor_nome} onChange={update("reitor_nome")} style={{ width: "100%", marginBottom: 10, padding: 8, borderRadius: 6, border: "1px solid #ddd" }} />
            <button className="uninter-btn-action" style={{ background: "#6366f1", width: "100%", marginBottom: 10 }} onClick={() => gerarAssinatura("assReitorImg")}>Gerar Assinatura</button>
            <label className="uninter-btn-action" style={{ background: "#94a3b8" }}>
              <Upload size={12} /> Upload Imagem
              <input type="file" accept="image/*" style={{ display: "none" }} onChange={handleUploadImg("assReitorImg")} />
            </label>
            {data.assReitorImg && <img src={data.assReitorImg} style={{ height: 40, marginTop: 10, border: "1px solid #eee" }} />}
          </div>

          {/* Assinatura Registro (Verso) */}
          <div style={{ padding: 15, border: "1px solid #e2e8f0", borderRadius: 12 }}>
            <label style={{ fontSize: 12, fontWeight: 700, display: "block", marginBottom: 10 }}>Registrador(a) (Verso)</label>
            <input type="text" value={data.secretaria_registro_nome} onChange={update("secretaria_registro_nome")} style={{ width: "100%", marginBottom: 10, padding: 8, borderRadius: 6, border: "1px solid #ddd" }} />
            <button className="uninter-btn-action" style={{ background: "#6366f1", width: "100%", marginBottom: 10 }} onClick={() => gerarAssinatura("assRegistroImg")}>Gerar Assinatura</button>
            <label className="uninter-btn-action" style={{ background: "#94a3b8" }}>
              <Upload size={12} /> Upload Imagem
              <input type="file" accept="image/*" style={{ display: "none" }} onChange={handleUploadImg("assRegistroImg")} />
            </label>
            {data.assRegistroImg && <img src={data.assRegistroImg} style={{ height: 40, marginTop: 10, border: "1px solid #eee" }} />}
          </div>
        </div>

        {/* PREVIEW OCULTO PARA EXPORTAÇÃO */}
