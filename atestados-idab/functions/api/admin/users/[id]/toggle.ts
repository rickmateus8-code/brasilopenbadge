...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) => {
  try {
    ...
    const user = await env.DB.prepare('SELECT id, username, is_active FROM users WHERE id = ? LIMIT 1').bind(userId).first<any>();
    if (!user) {
      return new Response(JSON.stringify({ success: false, error: 'Usuário não encontrado' }), { status: 404, headers: corsHeaders });
    }

    const nextActive = Number(user.is_active) ? 0 : 1;
    await env.DB.prepare('UPDATE users SET is_active = ?, updated_at = datetime("now") WHERE id = ?').bind(nextActive, userId).run();
    await logAdminAction(env, admin.id, 'toggle_user_active', userId, { previous: user.is_active, next: nextActive, username: user.username });

    return new Response(JSON.stringify({ success: true, is_active: nextActive }), { headers: corsHeaders });
  } catch (error: any) {
